...
const LoginSignup = () => {
  ...
  useEffect(() => {
    ...
    if (invite) {
      ...
      toast(`You're signing up via referral code: ${invite}`, {
        icon: 'ℹ️',
        style: {
          background: '#3b82f6',
          color: '#fff',
  ...
  const handleVerifyOTP = async (otp) => {
    try {
      // ✅ Pass invite code to login (which calls authService.verifyOTP)
      await login(mobileNo, otp, inviteCode); // ← PASS INVITE CODE
      toast.success('Login successful!');
      setShowOTPModal(false);

      // ✅ If signed up via invite, show success message
      if (inviteCode) {
        toast.success('Successfully linked to Channel Partner!', { duration: 4000 });
      }

      const pendingApplication = localStorage.getItem('pending_cp_application');
      if (pendingApplication) {
        toast('Completing your CP application...', {
          icon: 'ℹ️',
          duration: 3000,
          style: {
            background: '#3b82f6',
